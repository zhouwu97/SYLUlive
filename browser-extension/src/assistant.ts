...
if (!isProvider(provider)) {
  ...
  // esbuild 以 iife 格式打包扩展页面脚本，顶层 await 会让构建直接失败；
  // 把 await 之后的全部装配收进一个异步闭包，保持「先读到体测开关状态，
  // 再挂授权与登录监听」的原有顺序，风险确认框不会越过读取结果抢先响应点击。
  void (async () => {
    if (physicalRisk) {
      ...
      physicalRisk.addEventListener("change", async () => {
        try {
          ...
          status.textContent =
            error instanceof Error ? error.message : "体测开关更新失败";
        }
      });
    }
