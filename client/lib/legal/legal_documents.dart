...
class LegalDocuments {
  static const version = '2026-07-18-r2';
...
三、教务 WebView
教务网页仅在你主动使用相关功能时加载，并按专项授权使用必要的登录状态。新身份绑定只保存确认状态；本机直连时学校会话材料保存在设备安全存储或内存中。旧兼容部署可能仍有远端凭据策略，你可以在教务功能中清除本机会话或解除绑定。
...
你授权本服务在你主动注册、绑定或刷新教务功能时，使用你输入的教务账号和密码完成身份认证。新身份路径只保存经核验的教务身份和绑定状态，不保存学校密码、登录 Cookie 或会话令牌；本机直连所需的会话材料仅按你的设备设置保存在本机安全存储中，以读取你主动请求且已接入的教务数据。旧兼容部署可能仍保存必要的远端凭据。
