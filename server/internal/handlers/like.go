...
func (h *LikeHandler) UnlikePost(c *gin.Context) {
	...
	h.db.Where("user_id = ? AND target_type = ? AND target_id = ?", userID, "post", postID).Delete(&models.Like{})
...
func (h *LikeHandler) UnlikeReply(c *gin.Context) {
	...
	h.db.Where("user_id = ? AND target_type = ? AND target_id = ?", userID, "reply", replyID).Delete(&models.Like{})
	c.JSON(http.StatusOK, gin.H{"message": "已取消点赞"})
}