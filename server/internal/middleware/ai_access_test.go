...
func TestAIAccessMiddlewareEnforcesSwitchAndWhitelist(t *testing.T) {
	...
	tests := []struct {
		...
		{"总开关关闭", false, true, 18, http.StatusServiceUnavailable},
		{"非白名单用户", true, true, 19, http.StatusForbidden},
		{"白名单用户", true, true, 18, http.StatusNoContent},
		{"正式开放", true, false, 19, http.StatusNoContent},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			router := gin.New()
			router.Use(func(c *gin.Context) { c.Set("user_id", tt.userID) })
