...
func (v *CompetitionCatalogValidator) validateRecord(
	...
	if !containsCatalogEnum(record.RecommendationPermissionLevel, "blocked", "low", "medium", "high") {
		add("invalid_permission_level", id, "recommendation_permission_level", "推荐权限级别枚举无效")
	}
	if !containsCatalogEnum(record.AIMode, "disabled", "catalog_only", "candidate_explanation", "selected_comparison") {
