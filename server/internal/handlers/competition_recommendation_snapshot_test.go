...
func TestCreateCompetitionRecommendationSnapshotUsesServerResult(t *testing.T) {
	...
	if snapshot.EventVersion != 3 || snapshot.EventTitle != event.Title || snapshot.PersonalizedScore == nil {
		t.Fatalf("unexpected snapshot: %+v", snapshot)
	}
	if snapshot.RecommendationTier == "" || len(snapshot.FitReasons) == 0 || len(dto.FitReasons) == 0 {
