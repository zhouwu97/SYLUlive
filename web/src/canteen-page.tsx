  ...
  return <><Head title="食堂与菜品" description="食堂评分、菜品、排行与贡献"><button className="btn" onClick={()=>auth.requireUser()&&ui.open('我的点评',<MyReviews/>)}>我的点评</button><button className="btn primary" onClick={()=>change('tab','stores')}>＋ 写点评</button></Head>
    ...
    {tab==='mine'?(auth.user?<CanteenContributions/>:<Empty title="登录后查看自己的贡献"><button className="btn" onClick={auth.login}>登录</button></Empty>):<>
      ...
      <QueryState query={q}>{tab==='home'?<>
        ...
        <section className="panel panel-pad"><h3>今日热门</h3>{rows(q.data,'hot_dishes').map(d=><button className="list-row" key={d.id} onClick={()=>ui.open(d.name,<DishDetail canteenId={String(d.canteen_id)} id={d.id}/>)}><b>{d.name}</b><span>{d.canteen_name}</span></button>)}</section></div>
        <section className="section"><div className="section-head"><h2>今天吃什么</h2><button className="link-btn" onClick={()=>change('tab','stores')}>查看全部店铺</button></div><div className="dish-list">{rows(q.data,'feed').map(f=><Link className="dish" key={f.id} to={`/canteen/${f.canteen_id}`}>{f.image&&<img src={asset(f.image)} alt=""/>}<div><h4>{f.dish_name||f.canteen_name}</h4><p>{f.reason||f.title}</p></div></Link>)}</div></section>
        <section className="section"><h2>最新点评</h2><div className="panel">{rows(q.data,'recent_reviews').map(r=><Link className="list-row" key={r.id} to={`/canteen/${r.canteen_id}`}><div><b>{r.user_name||r.canteen_name}</b><p>{r.comment}</p></div><span>{r.overall_score} 分</span></Link>)}</div></section>
      </>:<><div className="panel">{rows(q.data,'canteens').map((c,i)=><Link className="list-row" to={`/canteen/${c.id}`} key={c.id}>{tab==='ranking'&&<span className="rail-index">{c.rank||i+1}</span>}{c.image&&<img className="author-avatar" src={asset(c.image)} alt=""/>}<div className="list-main"><b>{c.name}</b><p>{c.location_area} {c.location_floor} · {c.description}</p></div><span className="tag">{c.rating_count?`${Number(c.average_star).toFixed(1)} 分`:'暂无评分'}</span></Link>)}</div>{!rows(q.data,'canteens').length&&<Empty/>}{tab==='stores'&&<Pagination page={page} hasMore={rows(q.data,'canteens').length===20} onChange={n=>change('page',String(n))}/>}</>}
      </QueryState></>}
  </>;
}
