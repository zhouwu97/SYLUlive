...
**目标：** 在聊天表情面板中增加账号级云同步的自定义表情功能，首期支持 JPG、PNG、GIF，并支持长按聊天图片加入表情包；所有自定义表情在入库前压缩，单账号压缩后总空间上限为 50 MB。
...
- 自定义表情入库前必须压缩，账号按压缩后的文件大小累计，最多占用 50 MB。
...
网格单元的命中区域不小于 44×44 logical px。自定义 GIF 使用静态缩略图，避免面板同时解码大量动画；输入栏预览和发送消息使用压缩后的 GIF。
...
2. 打开系统图片选择器。
3. 校验文件格式、文件大小和图片尺寸。
4. 客户端显示“处理中”，服务端执行规范化压缩。
5. 服务端按压缩后的文件大小检查账号剩余空间。
6. 调用收藏接口创建用户表情资源和收藏关系。
7. 成功后将新内容插入收藏列表第一位。

默认限制：账号最多 80 个收藏项（内置表情和自定义表情合计），原始上传请求不超过 10 MB，单个压缩后自定义文件不超过 10 MB，账号自定义表情资源总容量不超过 50 MB。容量按服务端最终保存的压缩文件计费，限制由服务端配置控制，客户端只提供提前提示，服务端负责最终校验。

### 3.3 图片压缩规则

压缩在服务端执行，客户端可以先做轻量压缩以减少上传流量，但不能跳过服务端处理。服务端只保留规范化后的文件，不保留未压缩原文件：

- 静态 JPG/JPEG：最长边缩放到 512px，JPEG 质量固定为 82。
- 静态 PNG：有透明通道时保留 PNG 并使用最高级别压缩；无透明通道时转换为 JPEG，质量固定为 82。
- GIF：保留动画，逐帧缩放到最长边 512px，保留每帧延时并重新编码；压缩后的 GIF 仍需不超过单文件上限。
- 服务端返回 `compressed_size` 和 `quota_used`，客户端据此刷新容量提示。
- 压缩结果超过账号剩余 50 MB 空间时，整次添加事务回滚，不产生孤儿文件或收藏记录。
...
现有上传处理器允许 GIF，但现有图片变体逻辑不会为 GIF 生成缩略图。表情资源服务先压缩 GIF，再生成缩略图：

1. 使用 `image/gif` 解码全部帧并按最长边 512px 重新编码。
2. 从压缩后的 GIF 解码第一帧。
3. 按最长边 480px 生成 PNG 首帧缩略图。
4. 将缩略图路径写入 `UserEmojiAsset.ThumbnailPath`。
5. 压缩或缩略图生成失败时拒绝创建收藏资源并返回可读错误。

压缩后的 GIF 保留 `image/gif` MIME 类型和动画内容，接收方通过压缩后的文件发送和播放；未压缩原 GIF 不保留。
...
- 清理文件时按 `models.File.Size` 使用压缩后的实际字节数回收账号配额。
...
上传：idle / picking / compressing / uploading / creating / success / failed / quota-exceeded
...
- 从消息收藏会检查权限。
- GIF 能生成首帧缩略图。
- 静态图片会按规则缩放和压缩，透明 PNG 不丢失透明通道。
- GIF 重新编码后仍保留帧顺序和帧延时。
- 缩略图失败不会产生半条收藏记录。
- 删除收藏只减少引用，不删除仍被消息使用的文件。
- 达到 80 个收藏项、单个压缩文件 10 MB、总容量 50 MB 时返回明确错误。
- 删除自定义表情后释放其压缩文件大小，释放后的空间可以立即添加新表情。
...
3. 打开云端同步开关，验证新增、删除、长按添加、压缩计费和 GIF 发送。
