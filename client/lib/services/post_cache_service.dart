...
class PostCacheService {
  ...
  /// 保存指定板块的帖子列表到本地缓存
  static Future<void> savePosts(int boardId, List<Post> posts) async {
    final box = await _openBox();
    final key = '$_boardPrefix$boardId';
    final json = jsonEncode(posts.map((p) => _postToJson(p)).toList());
    await box.put(key, json);
  }

  /// 从本地缓存读取指定板块的帖子列表
  static Future<List<Post>> loadPosts(int boardId) async {
    final box = await _openBox();
    final key = '$_boardPrefix$boardId';
  ...
  static Future<String?> getLatestTimestamp(int boardId) async {
    final posts = await loadPosts(boardId);
  ...
  /// 合并新帖子到缓存（新帖在前，去重）
  static Future<void> mergeNewPosts(int boardId, List<Post> newPosts) async {
    if (newPosts.isEmpty) return;
    final existing = await loadPosts(boardId);
    ...
    await savePosts(boardId, merged);
  }
