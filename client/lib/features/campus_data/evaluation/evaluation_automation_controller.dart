...
class EvaluationAutomationController extends ChangeNotifier {
  ...
  Future<void> resumeBatch() async {
    ...
    _updateProgress(
        ...
        state: EvaluationAutomationState.switchingNext,
        message: '继续处理...');
    final token = ++_currentToken;
  ...
  void _logStateTransition() {
    if (_progress.state != _lastLoggedState) {
      if (kDebugMode) {
        debugPrint(
            '[EvaluationFlow] ${_lastLoggedState.name} -> ${_progress.state.name} (${_progress.message ?? ''})');
      }
  ...
  Future<void> _batchLoop(int token) async {
    while (!_disposed && token == _currentToken) {
      if (_progress.pauseRequested) {
        _updateProgress(
            state: EvaluationAutomationState.paused, message: '已暂停');
        return;
      }
      ...
      if (_progress.completedCount >= _maxTotalItems) {
        _updateProgress(
            state: EvaluationAutomationState.completed,
            message: '已达到最大处理上限 ($_maxTotalItems)');
        return;
      }
      ...
      if (nextItem == null) {
        ...
        _updateProgress(
            state: EvaluationAutomationState.completed,
            message: '所有可用待评项已处理完成');
        return;
      }

      // Wait for the form to load after selection
      _updateProgress(
          state: EvaluationAutomationState.waitingForForm,
          message: '等待表单加载...');
      ...
      // Execute Single Flow
      await _runSingleItemFlow(token,
          expectedFingerprint: nextItem.fingerprint);
  ...
  Future<void> _runSingleItemFlow(int token,
      {String? expectedFingerprint}) async {
    // 1. Probe
    _updateProgress(
        state: EvaluationAutomationState.probing, message: '正在解析页面...');
    ...
    final preSaveRaw =
        await ctrl.evaluateJavascript(source: buildSaveSnapshotScript());
    final preSave = EvaluationSaveSnapshot.fromJson(
        jsonDecode(preSaveRaw as String) as Map<String, dynamic>);
    ...
    _updateProgress(
        currentItemLabel: fingerprint.isEmpty ? '当前课程' : fingerprint);

    // 3. Fill
    _updateProgress(
        state: EvaluationAutomationState.filling, message: '正在填写分值...');
    ...
    if (kDebugMode) {
      debugPrint(
          '[EvaluationFlow] filled ${fillRes.scoreInputCompletedCount}/${fillRes.scoreInputCount}');
    }

    // 4. Save
    _updateProgress(
        state: EvaluationAutomationState.saving,
        message: _submitMode ? '正在提交...' : '正在保存...');
    final rawSave = await ctrl.evaluateJavascript(
        source: _submitMode
            ? buildSubmitCurrentScript()
    ...
    _updateProgress(
        state: EvaluationAutomationState.waitingForSave,
        message: '等待保存成功确认...');
    ...
    _updateProgress(
        state: EvaluationAutomationState.switchingNext, message: '等待页面刷新...');
    await Future.delayed(const Duration(milliseconds: 2000));
  }

  Future<bool> _pollForSaveSuccess(int token, InAppWebViewController ctrl,
      EvaluationSaveSnapshot preSave) async {
    ...
    while (DateTime.now().difference(startTime) < maxDuration) {
      ...
      final raw =
          await ctrl.evaluateJavascript(source: buildSaveSnapshotScript());
      if (raw != null) {
        final current = EvaluationSaveSnapshot.fromJson(
            jsonDecode(raw as String) as Map<String, dynamic>);

        // Success Conditions
        if (current.successMarkerCount > preSave.successMarkerCount)
          return true;
        if (current.savedCount > preSave.savedCount) return true;
        if (current.rowStatus != null &&
            current.rowStatus != preSave.rowStatus &&
            (current.rowStatus!.contains('已评') ||
                current.rowStatus!.contains('保存') ||
                current.rowStatus!.contains('提交'))) {
          return true;
        }
      }
  ...
  Future<EvaluationListItem?> _findAndSelectNextPendingItem(int token) async {
    ...
    while (pagesSearched < maxPagesToSearch) {
      ...
      final rawList =
          await ctrl.evaluateJavascript(source: buildGetEvaluationListScript());
      if (rawList == null) return null;

      final Map<String, dynamic> listData =
          jsonDecode(rawList as String) as Map<String, dynamic>;
      if (listData['error'] != null) {
        if (kDebugMode)
          debugPrint('[EvaluationFlow] list error: ${listData['error']}');
        return null;
      }

      final itemsRaw = listData['items'] as List<dynamic>? ?? [];
      final items = itemsRaw
          .map((e) => EvaluationListItem.fromJson(e as Map<String, dynamic>))
          .toList();
      ...
      for (final item in items) {
        ...
        if (isTarget && !_processedFingerprints.contains(item.fingerprint)) {
          // Select it
          final rawSel = await ctrl.evaluateJavascript(
              source: buildSelectNextPendingScript(item.rowId));
      ...
      final nextResRaw =
          await ctrl.evaluateJavascript(source: buildGoToNextPageScript());
  ...
  Future<bool> _waitForForm(int token, String expectedFingerprint) async {
    ...
    while (DateTime.now().difference(startTime) < maxDuration) {
      ...
      if (webViewController.currentPageType ==
          EvaluationPageType.evaluationForm) {
