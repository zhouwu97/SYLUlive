...
- 删除配置会同时清除该配置的 API Key；阶段 1 不持久化能力探测结果或第三方聊天上下文，因此没有可遗留的对应缓存。
...
- `GET /models` 只用于发现模型名称，聊天、流式和 Tool Calling 能力均标记为未知或关闭，不会因 HTTP 成功而被声明为可用。普通聊天固定 `stream: false`、`tools: []`，模型返回的工具 JSON 只会作为文本显示。
- 普通聊天记录只保留在当前页面内存中，切换模型设置或账号上下文关闭后立即清空，不做本地持久化。校园公益 AI 与自定义模型分别持有服务端和本地上下文；自定义模型消息由客户端直接发送到用户配置的模型服务，不经过沈理校园服务器。
...
- 提交前已执行本次修改文件的 `flutter analyze`（0 问题）与 `flutter test --no-pub`（546 passed）；端点、账号隔离、空工具列表、SSE 完整性和退出超时均有独立回归测试。
