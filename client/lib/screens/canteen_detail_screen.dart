...
class CanteenDetailScreen extends StatefulWidget {
  ...
  const CanteenDetailScreen({super.key, required this.canteenId, required this.canteenName});
  ...
  Widget _buildRatingCard(Map<String, dynamic> r, bool isDark) {
    ...
    if (r['images'] != null) {
      if (r['images'] is String && r['images'].toString().startsWith('[')) {
        try {
          final List<dynamic> decoded = r['images'] != null ? List<dynamic>.from(r['images'].toString().replaceAll('[', '').replaceAll(']', '').replaceAll('"', '').split(',').map((e) => e.trim()).where((e) => e.isNotEmpty)) : [];
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const SizedBox(width: 8),
            Expanded(child: Text(r['user_name'] ?? '匿名', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14))),
        ...
        Row(children: [
          ...
          if (!_editing) TextButton(onPressed: () => setState(() {
            ...
            if (myRating != null) { 
              ...
              _ratingImages = [];
              try {
                if (myRating['images'] != null && myRating['images'].toString().startsWith('[')) {
        ...
        if (!_editing && myRating != null) ...[
          ...
          Row(mainAxisAlignment: MainAxisAlignment.end, children: [
            ...
            ElevatedButton(onPressed: _star == 0 ? null : () async {
              final success = await context.read<CanteenProvider>().rateCanteen(widget.canteenId, _star, _commentCtrl.text, _ratingImages);
              if (mounted) {
