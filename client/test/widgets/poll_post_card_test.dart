...
void main() {
  testWidgets('多选卡片最多保留最大选择数', (tester) async {
    await tester.pumpWidget(ChangeNotifierProvider(
        ...
        child: MaterialApp(home: Scaffold(body: PollPostCard(post: _post())))));
  ...
  testWidgets('提交成功通过回调立即更新宿主', (tester) async {
    ...
    await tester.pumpWidget(ChangeNotifierProvider(
        ...
        child: MaterialApp(
            home: Scaffold(
                body: PollPostCard(
                    post: _post(multiple: false),
                    onPostUpdated: (post) => updated = post)))));
    await tester.tap(find.text('选项一'));
    await tester.pump();
