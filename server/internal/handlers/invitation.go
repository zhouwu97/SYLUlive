...
func (h *InvitationHandler) GetPending(c *gin.Context) {
	...
	if err := h.db.Where("user_id = ? AND status = ?", userID, models.InvitationStatusPending).
...
func (h *InvitationHandler) Approve(c *gin.Context) {
	...
	if input.Reject {
		...
		if err := h.db.Model(&invitation).Update("status", models.InvitationStatusRejected).Error; err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "数据库操作失败"})
			return
		}
