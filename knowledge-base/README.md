...
1. 在新的版本目录中加入来源原文、来源台账、确定性规则和测试用例，不修改已经发布的版本目录。
2. 为每份资料记录来源 URL、发布日期、抓取时间、内容哈希和适用范围；无法核验的材料标记为 `needs_review`。
3. 运行完整性检查和 RAG 离线回归，确认引用、历史版本提示和缺失信息拒答行为。
4. 通过管理员发布流程后，再把发布 manifest 和生产状态写回版本目录；仓库文件本身只表示候选/审计材料，不表示已经上线。
