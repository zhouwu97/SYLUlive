  ...
  group('FitnessWeeklyPlanEngine', () {
    ...
    test('过期体测仅保守运行，极端 BMI 停止具体安排', () {
      final plan = FitnessWeeklyPlanEngine().build(
        ...
        weightKg: 120,
        physicalOverviewAvailable: true,
        physicalOverviewStale: true,
      );
      expect(plan.sessions, isEmpty);
      expect(plan.safetyNotes.join(), contains('BMI 处于高风险范围'));
      expect(plan.safetyNotes.join(), contains('体测数据已过期'));
    });
  });
}
