...
func TestEduCredentialCleanupJobServiceRetriesWhenLocalPlaceholderDeletionFails(t *testing.T) {
	db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
	if err != nil {
		t.Fatalf("open db: %v", err)
	}
	...
	if err := db.Exec(`
		CREATE TRIGGER block_registration_cleanup
		BEFORE DELETE ON users
		WHEN OLD.id = 46
		BEGIN
			SELECT RAISE(FAIL, 'simulated placeholder delete failure');
		END;
	`).Error; err != nil {
	...
	firstReport, err := service.ProcessDue(context.Background(), 10)
	if err != nil {
		t.Fatalf("process failed local deletion: %v", err)
	}
	if firstReport.Processed != 1 || firstReport.Failed != 1 || firstReport.Completed != 0 {
		t.Fatalf("failed local deletion was incorrectly completed: %#v", firstReport)
	}
	var failed models.EduCredentialCleanupJob
	if err := db.Where("user_id = ?", user.ID).First(&failed).Error; err != nil {
		t.Fatalf("load failed cleanup job: %v", err)
	}
	if failed.CompletedAt != nil || failed.Attempts != 1 || failed.LastError == "" {
		t.Fatalf("failed cleanup job is not retryable: %#v", failed)
	}
	if err := db.Exec("DROP TRIGGER block_registration_cleanup").Error; err != nil {
		t.Fatalf("remove delete failure trigger: %v", err)
	}
	now = failed.NextAttemptAt

	secondReport, err := service.ProcessDue(context.Background(), 10)
	if err != nil {
		t.Fatalf("process retried local deletion: %v", err)
	}
	if secondReport.Processed != 1 || secondReport.Completed != 1 || secondReport.Failed != 0 {
		t.Fatalf("retried local deletion did not complete: %#v", secondReport)
	}
	var remaining int64
	if err := db.Model(&models.User{}).Where("id = ?", user.ID).Count(&remaining).Error; err != nil {
		t.Fatalf("count deleted placeholder user: %v", err)
	}
	if remaining != 0 || len(remote.userIDs) != 2 {
		t.Fatalf("placeholder deletion was not retried: remaining=%d calls=%v", remaining, remote.userIDs)
	}
}
