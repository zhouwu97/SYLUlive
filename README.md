...
**💻 平板与大屏体验专项优化 (Pad UI Adaptation)**
- **无缝分屏与沉浸背景**：在平板宽屏模式下，点击集市与首页水贴详情，右侧将平滑展开为双窗格分屏布局。对于分屏状态下的左侧列表，引入了专属的垂直居中半透明壁纸，彻底消除页面割裂感。
- **课表网格精准对齐与动态排版**：大屏课表全面重构，使用精准可用空间实时分配网格，消除因为侧边栏导致的单元格挤压错位。此外，加入了**动态智能字体缩放（最高1.35倍）**与动态内边距机制，屏幕再大内容也不会显得空旷。
- **全局组件透明化与留白优化**：宽屏下左侧的侧边导航栏（NavigationRail）已全面适配 Glassmorphism 半透明毛玻璃风格；底部悬浮胶囊栏进行“瘦身”处理，释放更多阅读空间。
- **集市层级体验修复**：修复了由集市跳转帖子详情时偶发的返回键丢失和排版挤压问题。
