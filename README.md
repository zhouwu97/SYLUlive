...
- **自动答题智能降级匹配**：重写了网页端注入 JS 脚本的物理点击逻辑。在纯图片选项无文字的情况下，能智能降级比对题号字母（A/B/C/D）进行精准命中，彻底解决选项不包含文字时无法自动点击的痛点。
- **答案快捷复制**：雨课堂答案外挂控制台新增“一键复制”按钮与长按自由框选功能，方便手动输入。
- **手机端 UI 拉伸修复**：修复了在部分手机（如荣耀）特殊分辨率及全面屏手势下，应用被强行开启兼容模式导致的整体纵向拉伸变形、头像变椭圆的问题；彻底修复了底部导航栏在窄屏手机上的溢出与不居中问题。

### v1.4.35
- **UI & 体验**：深度优化平板(Pad)/横屏设备的界面适配。
- **自定义背景**：新增横屏专属自定义背景功能，彻底解决横屏/分屏模式下背景图拉伸或只显示一半的问题，并全局适配半透明毛玻璃效果。
- **今日概览**：修复首页“今日概览”日期和星期不随课表周数切换而更新的问题。
- **课表布局**：优化课表页卡片在宽屏下的边距与字体缩放，修正了“返回本周”按钮宽屏过宽的问题。

### v1.3.0
- **全新界面交互**：支持全局亮暗色模式切换、动态主题色提取。
- 教务账号绑定与 Token 态维持
