...
class YuketangWebViewWidgetState extends State<YuketangWebViewWidget> {
  ...
  Map<String, dynamic>? _lastQuestionPayload;
  String _lastQuestionType = '';
  String _lastContentText = '';
  AnswerGatewayResult? _lastAnswerResult;
  bool _canConfirmCache = false;
            ...
            onPanUpdate: (details) {
              if (mounted)
                setState(() {
                  ...
                  // 简单边界限制
                  if (_dashboardPos.dx < 0)
                    _dashboardPos = Offset(0, _dashboardPos.dy);
                  if (_dashboardPos.dy < 0)
                    _dashboardPos = Offset(_dashboardPos.dx, 0);
                  final double maxX = MediaQuery.of(context).size.width - 200;
                  final double maxY = MediaQuery.of(context).size.height - 100;
                  if (_dashboardPos.dx > maxX)
                    _dashboardPos = Offset(maxX, _dashboardPos.dy);
                  if (_dashboardPos.dy > maxY)
                    _dashboardPos = Offset(_dashboardPos.dx, maxY);
                });
            },
            child: Container(
  ...
  Future<void> _handleUpload() async {
    ...
    if (_lastUploadTime != null &&
        now.difference(_lastUploadTime!).inMinutes >= 30) {
      _uploadedIndices.clear();
      _uploadedAll = false;
    }

    String rangeStr = _isBlockMode
        ? (_selectedBlock ?? '')
        : (_selectedSingle?.toString() ?? '');
    ...
    if (mounted) {
      setState(() {
        ...
        _answerText = '🤖 AI答题中，请稍候...';
        _canConfirmCache = false;
      });
    }
    ...
    try {
      ...
      final questionType = data['type']?.toString() ?? '未知题型';
      final contentText = data['content']?.toString() ?? jsonString;

      final answerResult = await _answerGateway.processQuestion(
        data,
        onProgress: (status) {
      ...
      if (mounted) {
        _lastQuestionPayload = data;
        _lastQuestionType = questionType;
        _lastContentText = contentText;
        _lastAnswerResult = answerResult;

        if (answerResult.ok) {
          setState(() {
            _statusText = answerResult.summaryText;
            _answerText = answerResult.answer;
            _canConfirmCache = false;
          });
          final safeAnswer = answerResult.answer
              .replaceAll("'", "\\'")
              .replaceAll("\n", "\\n");
          ...
          setState(() {
            _statusText = '错误';
            _answerText = answerResult.message;
            _canConfirmCache = false;
          });
        }
      }
  ...
  Future<void> _handleRetryWithFreshAi() async {
    if (_lastQuestionPayload == null ||
        _lastAnswerResult == null ||
        !_lastAnswerResult!.usesOfficialBackend) {
      return;
    }

    if (mounted) {
      setState(() {
        _statusText = '正在标记错题并重新发起 AI 作答...';
        _answerText = '🤖 AI答题中，请稍候...';
        _canConfirmCache = false;
      });
    }

    final markMessage = await _answerGateway.markQuestionWrong(
      _lastQuestionType,
      _lastQuestionPayload!,
      _lastContentText,
    );
    if (mounted && markMessage != null && markMessage.isNotEmpty) {
      setState(() => _statusText = markMessage);
    }

    final freshResult = await _answerGateway.processQuestion(
      _lastQuestionPayload!,
      onProgress: (status) {
        if (mounted) setState(() => _statusText = status);
      },
    ...
    _lastAnswerResult = freshResult;
    if (freshResult.ok) {
      setState(() {
        _statusText = '✅ 已重新作答，请确认无误后写入缓存';
      ...
      setState(() {
        _statusText = '错误';
        _answerText = freshResult.message;
        _canConfirmCache = false;
      });
    }
  }

  Future<void> _handleConfirmCache() async {
    if (!_canConfirmCache ||
        _lastQuestionPayload == null ||
        _lastAnswerResult == null ||
        !_lastAnswerResult!.ok) {
      return;
    }

    if (mounted) {
      setState(() => _statusText = '正在写入官方缓存...');
    }

    final message = await _answerGateway.confirmCachedAnswer(
      _lastQuestionType,
      _lastQuestionPayload!,
      _lastContentText,
      _lastAnswerResult!.answer,
    );

    if (!mounted) return;

    setState(() {
      _statusText = message ?? '已写入缓存';
      _canConfirmCache = false;
    });
  }
}
