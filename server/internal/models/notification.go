...
const (
	...
	NotificationTypeFeedbackAdminUpdate   = "feedback_admin_update"
)
...
type Notification struct {
	...
	UserID    uint      `gorm:"not null;index;uniqueIndex:idx_notification_dedup,where:dedup_key <> ''" json:"user_id"`      // 接收通知的用户
	Type      string    `gorm:"size:50;not null;index;uniqueIndex:idx_notification_dedup,where:dedup_key <> ''" json:"type"` // 通知类型: reply
	Content   string    `gorm:"type:text" json:"content"`                                                                    // 通知内容摘要
	RelatedID uint      `gorm:"index" json:"related_id"`                                                                     // 关联对象ID（如回复ID）
	DedupKey  string    `gorm:"size:191;index;uniqueIndex:idx_notification_dedup,where:dedup_key <> ''" json:"-"`            // 业务事件去重键
