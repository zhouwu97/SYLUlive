...
- PR #45 已合并，并从 `origin/main@9ec0bb5d` 创建独立干净 worktree 重放客户端差异。
- 全量 `flutter test --no-pub`：655 项通过；AI Runtime 与 Vault 定向用例均包含在该总数内，不重复累计。
- 全量 `flutter analyze --no-pub --no-fatal-warnings --no-fatal-infos`：无 error；仓库既有 399 条 warning/info。
- GitHub PR #46 Actions：客户端、服务端、教务、RAG、pgvector 等全部检查通过。
- `git diff --check`：无空白错误，仅报告工作树原有 LF/CRLF 转换提示。
- 个人 AI 目录及相关 Provider 敏感字段、日志扫描：无命中。
- 正式 Skill 清单见 `docs/ai/client-ai-skill-catalog.md`，RC 覆盖矩阵见 `docs/ai/client-ai-rc-test-matrix.md`。
...
5. RC 分支由干净 `main` 建立，只包含客户端 AI、对应测试和交付文档；原工作树的服务端、部署和 RAG 修改未被带入。
