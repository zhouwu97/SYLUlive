...
void main() {
  test('固定下发十二项 Tool 且全部拒绝额外参数', () {
    final definitions = buildStageSixToolDefinitions();
    expect(definitions, hasLength(12));
    expect(definitions.map((item) => item.id).toSet(), hasLength(12));
    for (final definition in definitions) {
      expect(definition.parameters['type'], 'object');
      expect(definition.parameters['additionalProperties'], isFalse);
      expect(
        definition.parameters.toString().toLowerCase(),
        isNot(anyOf(contains('student_id'), contains('api_key'))),
      );
    }
  });
    ...
    test('低敏感允许会话复用，中敏感强制降级为仅本次', () async {
      ...
      expect(
        await manager.authorize(
          _preview(SkillSensitivity.low, providerScope: 'provider-b'),
        ),
        ToolPermissionDecision.allowSession,
      );
      expect(
        await manager.authorize(
          _preview(SkillSensitivity.low, accountScope: 'account-b'),
        ),
        ToolPermissionDecision.allowSession,
      );
  ...
  group('LocalToolLoop', () {
    test('授权后执行最小化 Skill 并把证据回传模型', () async {
      ...
      final loop = _loop(model: model, audit: audit, prompt: prompt);
      ...
      final sentPayload = model.receivedMessages.last.content;
      expect(
        prompt.lastPreview?.payloadHash,
        sha256.convert(utf8.encode(sentPayload)).toString(),
      );
      expect(prompt.lastPreview?.payloadSize, sentPayload.length);
    });

    test('未授权只读取本地最小化载荷并向模型回传结构化拒绝', () async {
      final gateway = _Gateway();
      final model = _ScriptedModel(<ToolModelTurn>[
        ToolModelTurn.call(_academicCall('call-1')),
        const ToolModelTurn.finalAnswer('已尊重用户拒绝。'),
      ]);
      final loop = _loop(
        model: model,
        gateway: gateway,
      ...
      expect(model.turnsRequested, 2);
      expect(
          model.receivedMessages.last.content, contains('permission_denied'));
      expect(model.receivedMessages.last.content, isNot(contains('evidence')));
    });
    ...
    test('拒绝未知 Tool、重复调用和超过三轮的调用', () async {
      ...
      final duplicateId = _ScriptedModel(<ToolModelTurn>[
        ToolModelTurn.call(_academicCall('same-id')),
        ToolModelTurn.call(
          LocalToolCall(
            id: 'same-id',
            tool: AcademicOverviewSkill.skillId,
      ...
      expect(
        (await _loop(model: duplicateId).run(
          userMessage: '重复 ID',
          tools: buildStageSixToolDefinitions(),
        ))
            .warnings
    ...
    test('同一轮拒绝后相同 Skill 不得再次弹出授权', () async {
      final prompt = _Prompt(ToolPermissionDecision.denied);
      final model = _ScriptedModel(<ToolModelTurn>[
        ToolModelTurn.call(_academicCall('first')),
        ToolModelTurn.call(_academicCall('second')),
      ]);
      final outcome = await _loop(model: model, prompt: prompt).run(
        userMessage: '再次申请',
        tools: buildStageSixToolDefinitions(),
      );
      expect(outcome.status, ToolLoopStatus.rejected);
      expect(outcome.warnings.single, contains('禁止重复申请'));
      expect(prompt.count, 1);
    });
...
ToolPermissionPreview _preview(
  ...
  String accountScope = 'account-a',
  String providerScope = 'provider-a',
}) =>
    ToolPermissionPreview(
      ...
      outputFields: const <String>['course_count'],
...
class _ScriptedModel implements ToolCallingModel {
  ...
  String get authorizationScope =>
      'test-provider|https://example.test/v1|model';
