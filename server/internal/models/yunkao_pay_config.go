...
type YunkaoPayOrder struct {
	...
	GatewayPayURL   string     `gorm:"type:text" json:"-"`                       // MAPI 返回的网页支付地址
	GatewayQRCode   string     `gorm:"type:text" json:"-"`                       // MAPI 返回的真实付款码内容
	GatewayScheme   string     `gorm:"type:text" json:"-"`                       // MAPI 返回的客户端唤起地址
