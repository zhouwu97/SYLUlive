...
class LegalDocuments {
  static const version = '2026-09-22-r3';
...
三、分项授权与账号注销
教务绑定、远程消息推送和本地提醒分别管理，可在对应功能入口关闭或撤回。你也可以使用密码确认注销账号，注销后身份资料会被清除或匿名化。
...
二、处理目的与最小化
账号信息用于身份认证和服务提供；设备推送标识仅用于消息提醒；教务数据仅用于已启用的教务功能。服务端 verify-only 核验时，教务密码仅在本次 HTTPS 请求中经由服务端用于学校身份核验，不持久化保存。密码、Cookie 和会话令牌不出现在个人数据导出文件中。
...
一、极光推送
远程消息推送默认关闭。只有你主动开启并确认推送处理摘要后，应用才初始化极光 SDK、获取 Registration ID 并上传设备标识。服务端仅按已认证账号登记的设备 Registration ID 投递消息，不使用用户可自行声明的 Alias。当前版本一个账号仅支持一台设备接收远程消息，后启用设备会替换此前设备；关闭入口在“消息与通知”。

二、图片选择与裁剪
仅在你主动上传头像、帖子或商品图片时请求相册访问。图片会在上传前由你选择，上传后用于展示你发布的内容。

三、教务 WebView
教务网页仅在你主动使用相关功能时加载，并按专项授权使用必要的登录状态。本机直连时学校会话材料保存在设备安全存储或内存中；服务端 verify-only 核验时，密码仅在本次 HTTPS 请求中经过服务端且不持久化保存。新身份绑定只保存核验后的身份与绑定状态。旧兼容部署可能仍有远端凭据策略，你可以在教务功能中清除本机会话或解除绑定。
...
教务登录和身份核验会按你选择的功能使用不同凭据路径：

一、本机直连。设备直接向学校系统提交教务账号和密码。学校会话按你的设置保存在本机安全存储或内存中；云端绑定只同步教务类型与核验后的学号，不接收本机直连的密码或学校会话。

二、服务端 verify-only 身份核验。若当前功能使用此方式，教务密码会通过本次 HTTPS 请求发送至 SYLUlive 服务端，并仅用于本次学校身份核验；服务端不持久化保存该密码或学校会话。新身份路径只保存核验后的教务身份和绑定状态。

旧兼容部署可能仍保存提供既有功能所需的远端凭据。你可以在教务功能中清除本机会话或解除绑定。

处理目的仅限教务功能和身份核验，不用于公开展示、广告营销或向无关用户提供。你可以在教务功能入口清除本机会话或解除绑定；旧兼容部署的远端凭据清理失败时会进入重试任务。
''',
  );
