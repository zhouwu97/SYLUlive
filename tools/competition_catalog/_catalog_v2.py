...
def validate_document(document: Mapping[str, Any]) -> list[str]:
    ...
    for index, raw in enumerate(items, 1):
        ...
        if record.get("recommendation_permission_level") not in {"low", "medium", "high"}:
        ...
        if record.get("blocker_codes") and record.get("candidate_pool_allowed"):
            errors.append(f"{competition_id}: 存在阻断码时不能进入候选池")
