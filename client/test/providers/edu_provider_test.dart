...
void main() {
  ...
  group('EduProvider 本机教务契约', () {
    for (final choice in [null, false]) {
      test('首次初始化保留默认保存和明确关闭选择：$choice', () async {
        final fixture = await createFixture(saveAcademicData: choice);
        ...
        expect(snapshot != null, choice ?? true);
      });
    }
    ...
    test('没有本机会话控制器时不会回退到旧服务端教务接口', () async {
      ...
      expect(result.errorCode, 'ACADEMIC_SESSION_NOT_READY');
