  ...
  static List<GradeComponent> _parseGradeDetailJson(Object? payload) {
    ...
    if (payload is Map<String, dynamic>) {
      if (payload['items'] is List<dynamic>) {
        rows.addAll((payload['items'] as List<dynamic>).cast<Map<String, dynamic>>());
      } else if (payload['rows'] is List<dynamic>) {
        rows.addAll((payload['rows'] as List<dynamic>).cast<Map<String, dynamic>>());
      } else if (payload['data'] is List<dynamic>) {
        rows.addAll((payload['data'] as List<dynamic>).cast<Map<String, dynamic>>());
        ...
        for (final value in payload.values) {
          if (value is List<dynamic>) {
            rows.addAll(value.cast<Map<String, dynamic>>());
            break;
          }
        }
      }
    } else if (payload is List<dynamic>) {
      rows.addAll(payload.cast<Map<String, dynamic>>());
    }
    ...
    for (final row in rows) {
      ...
      final score = _firstNonEmpty(row, [
        'cj',
        'xmcj',
        'score',
        'df',
      ...
      final weight = _firstNonEmpty(row, [
        'bl',
        'xmbfb',
        'cjxmbl',
        'weight',
